...
class LinkedList:
    ...
    def remove(self, value):
        """
        Iterate through all of the nodes, until you find the node that has
        the value you're looking for. Once you find the node you're looking for
        set the next_node attribute for the previous node, to the node you found's next_node
        attribute. Effectively removing the node from the chain, as there is no longer
        any reference to the node.
        """
        current_node = self.head
        prev_node = None
    ...
    def insert_after(self, value, target_value=None):
        """
        Insert a new node with the provided value
        after the node with the same value as target_value.
        If there is not target_value, insert the new node to
        the end of the list.
        """
        new_node = Node(value)
